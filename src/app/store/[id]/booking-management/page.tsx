...
export default function BookingManagementPage() {
  ...
  const { 
    ...
    error: overviewError 
  } = useBookingOverview(storeId);
  
  const { 
    todaysBookings, 
    loading: todaysLoading, 
    error: todaysError 
  } = useTodaysBookings(storeId);
  
  const { 
    weeklyData: weeklyBookingData, 
    loading: weeklyLoading, 
    error: weeklyError 
  } = useWeeklyBookingData(storeId);

  // User must be authenticated to access bookings
  const canAccessBookings = user !== null;

  // Overall loading state
  const isLoading = overviewLoading || todaysLoading || weeklyLoading;

  useEffect(() => {
    console.log('🔄 Booking Management Data:', {
      shop: shop?.name || `Shop ${storeId}`,
      overviewLoading,
      bookingOverview,
  ...
  }, [shop, bookingOverview, todaysBookings, weeklyBookingData, overviewLoading, canAccessBookings, user, storeId]);
  ...
  const handleCalendarView = () => {
    router.push(`/store/${storeId}/calendar-view`);
  };
  ...
  // Access control - user must be authenticated to access booking management
