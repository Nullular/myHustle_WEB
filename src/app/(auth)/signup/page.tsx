...
export default function SignUpPage() {
  ...
  const onSubmit = async (data: SignUpForm) => {
    ...
    try {
      // Always lowercase the email before validation and signup
      const normalizedEmail = data.email.toLowerCase().trim();
      const emailValidation = await testerValidationService.validateTesterEmail(normalizedEmail);
      if (!emailValidation.isValid) {
        setError(emailValidation.message || 'This platform is only available to our loyal testers!');
        setIsLoading(false);
        return;
      }
      const user = await AuthService.signUp({ ...data, email: normalizedEmail });
      setUser(user);
